...
Now that's out of the way, Kubernetes is simply a technology to automatically manage Docker containers based on a set of rules, so it doesn't need to be done manually.  The name is derived from the Greek word <code>κυβερνήτης</code> (`kubernḗtēs`) which means pilot or helmsman, hence the Kubernetes logo being a ship's steering wheel:
